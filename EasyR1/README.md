...
## SeePhys Pro Reproduction Scripts
...
The `examples/repro/` directory contains the retained paper-reproduction launchers:
PhysRL physics RL and visual-math control RL, each with normal and blind
controls for Qwen3-VL-4B-Instruct and Qwen2.5-VL-7B-Instruct. Dataset ids are
sanitized placeholders and can be overridden with `TRAIN_FILES`, `VAL_FILES`,
`PHYSRL_TRAIN_FILES`, `PHYSICS_VAL_FILES`, `MATH_TRAIN_FILES`, or
`MATH_VAL_FILES`.
