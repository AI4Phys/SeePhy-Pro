...
def soft_overlong_punishment(response_length: int, max_response_length: int, overlong_buffer_length: int):
    ...
    if response_length <= expected_len:
        ...
        return (response_length - expected_len) / overlong_buffer_length
    else:
        return 0.0
...
def compute_score(reward_inputs: List[Dict[str, Any]], 
    ...
    for reward_input in reward_inputs:
        ...
        scores.append(
            {
                "overall": (1 - format_weight - overlong_penalty_factor) * accuracy_score + format_weight * format_score + overlong_score * overlong_penalty_factor,
                "format": format_score,
