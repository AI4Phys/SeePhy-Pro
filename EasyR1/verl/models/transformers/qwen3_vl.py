...
def qwen3_vl_patch_embed_forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
    ...
    # GH200 + cuDNN frontend may fail to find a bf16 Conv3D engine for Qwen3-VL patch embedding.
    # Match the NVIDIA forum workaround by running this Conv3D in fp16 and casting back.
    if target_dtype == torch.bfloat16 and hidden_states.is_cuda:
        hidden_states = hidden_states.contiguous().to(torch.float16)
        weight = self.proj.weight.to(torch.float16)
        bias = self.proj.bias.to(torch.float16) if self.proj.bias is not None else None
        hidden_states = F.conv3d(hidden_states, weight, bias, stride=self.proj.stride)
        return hidden_states.view(-1, self.embed_dim).to(target_dtype)
