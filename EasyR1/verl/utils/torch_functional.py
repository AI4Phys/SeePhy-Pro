...
def multivariate_whiten(
    data_matrix: torch.Tensor,
    eps: float = 1e-6,
    regularization: float = 1e-5
) -> tuple[torch.Tensor, torch.Tensor]:
    """
    Apply multivariate whitening transformation to remove correlations between dimensions.
    
    Given a data matrix X of shape (n_samples, n_dims), this function:
    1. Computes the mean vector μ and covariance matrix Σ
    2. Performs eigendecomposition: Σ = QΛQ^T
    3. Computes whitening matrix: W = QΛ^(-1/2)Q^T
    4. Applies transformation: Y = (X - μ)W^T
    
    Args:
        data_matrix: `(torch.Tensor)`
            shape: (n_samples, n_dims) - data matrix where each row is a sample
        eps: `(float)`
    ...
    Returns:
        whitened_data: `(torch.Tensor)`
            shape: (n_samples, n_dims) - whitened data with zero mean and identity covariance
        whitening_matrix: `(torch.Tensor)`
            shape: (n_dims, n_dims) - whitening transformation matrix
    """
    n_samples, n_dims = data_matrix.shape
    
    if n_samples < 2:
        # Not enough samples for covariance estimation, return identity transformation
        return data_matrix - data_matrix.mean(dim=0, keepdim=True), torch.eye(n_dims, device=data_matrix.device, dtype=data_matrix.dtype)
    
    # Compute mean
    mean = data_matrix.mean(dim=0, keepdim=True)  # (1, n_dims)
    centered_data = data_matrix - mean  # (n_samples, n_dims)
    
    # Compute covariance matrix
    # Σ = (1/(n-1)) * (X - μ)^T (X - μ)
    if n_samples > 1:
        cov_matrix = (centered_data.T @ centered_data) / (n_samples - 1)  # (n_dims, n_dims)
    else:
        cov_matrix = torch.zeros(n_dims, n_dims, device=data_matrix.device, dtype=data_matrix.dtype)
    
    # Add regularization to diagonal for numerical stability
    cov_matrix = cov_matrix + regularization * torch.eye(n_dims, device=data_matrix.device, dtype=data_matrix.dtype)
    
    # Eigendecomposition: Σ = QΛQ^T
    try:
        eigenvals, eigenvecs = torch.linalg.eigh(cov_matrix)  # eigenvals: (n_dims,), eigenvecs: (n_dims, n_dims)
        # Ensure eigenvalues are positive (add small epsilon)
        eigenvals = torch.clamp(eigenvals, min=eps)
        
        # Compute whitening matrix: W = QΛ^(-1/2)Q^T
        inv_sqrt_eigenvals = torch.rsqrt(eigenvals)  # (n_dims,)
        whitening_matrix = eigenvecs @ torch.diag(inv_sqrt_eigenvals) @ eigenvecs.T  # (n_dims, n_dims)
        
        # Apply whitening transformation: Y = (X - μ)W^T
        whitened_data = centered_data @ whitening_matrix.T  # (n_samples, n_dims)
        
    except Exception as e:
        # Fallback: if eigendecomposition fails, use diagonal normalization
        print(f"Warning: Eigendecomposition failed ({e}), using diagonal normalization")
        std_per_dim = torch.sqrt(torch.diag(cov_matrix)) + eps
        whitening_matrix = torch.diag(1.0 / std_per_dim)
