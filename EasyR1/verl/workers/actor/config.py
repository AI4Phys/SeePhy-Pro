...
class ActorConfig:
    ...
    """loss type: `default`, `gspo`, `gspo_token`, `gspo_token_penalize_low_entropy`, `gspo_token_penalize_window`, `gspo_token_encourage_entropy`, `gspo_token_flip_advantages`, `cispo`"""
    # Parameters for gspo_token_penalize_low_entropy
    entropy_threshold_ratio: float = 0.25
    """entropy threshold ratio for low entropy penalty (relative to average entropy)"""
    min_consecutive_length: int = 10
    """minimum consecutive low entropy tokens to trigger penalty"""
    # Parameters for gspo_token_penalize_window
    window_size: int = 64
    """size of sliding window for window-based low entropy penalty"""
    window_stride: int = 32
    """stride between windows for window-based low entropy penalty"""
    min_consecutive_windows: int = 3
    """minimum consecutive low entropy windows to trigger penalty"""
    penalty_factor: float = 0.5
    """factor to reduce importance score for penalized tokens"""
    # Parameters for gspo_token_encourage_entropy
    entropy_selection_ratio: float = 0.3
    """proportion of low entropy tokens to randomly select for entropy encouragement (0.0-1.0)"""
    entropy_encouragement_weight: float = 0.1
    """weight for entropy encouragement term. Higher values = stronger push towards higher entropy"""
    """factor to reduce importance score for penalized tokens"""
    ppo_epochs: int = 1
