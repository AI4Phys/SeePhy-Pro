...
class WorkerConfig:
    ...
    reward_model: Optional[RewardModelConfig] = None  # Optional: neural network-based reward model
    val_reward: Optional[RewardConfig] = None  # Optional: if None, use reward for validation
    ...
    def post_init(self):
        ...
        # If val_reward is not provided, use reward for validation
        # Note: We don't assign self.val_reward = self.reward here to avoid reference issues
        # Instead, we handle this in main.py when creating the reward manager
