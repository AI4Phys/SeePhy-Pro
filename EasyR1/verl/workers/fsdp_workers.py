...
from .reward.model_config import RewardModelConfig
...
class FSDPWorker(Worker):
    def __init__(
        self,
        config: WorkerConfig,
        role: Literal["actor", "critic", "rollout", "ref", "actor_rollout", "actor_rollout_ref", "reward"],
    ):
        super().__init__()
        self.config = config
        self.role = role
        ...
        self._use_reward_model_param_offload = False
        if self._has_actor:
        ...
        if self._has_reward_model:
            if self.config.reward_model is None:
                raise ValueError("Reward model role specified but reward_model config is None.")
            self._use_reward_model_param_offload = self.config.reward_model.offload.offload_params
            self._init_dist_mesh(self.config.reward_model, "reward")

    def _init_dist_mesh(self, config: Union[ActorConfig, CriticConfig, RewardModelConfig], role: Literal["actor", "critic", "reward"]):
        ...
        # validate and normalize config (skip for reward model as it doesn't need batch size validation)
        if role != "reward":
    ...
    def _build_model_optimizer(
        ...
        padding_free: bool,
        role: Literal["actor", "critic", "ref", "reward"],
    ) -> None:
        if role != "ref":  # ref model's tokenizer is same as actor, reward model needs its own tokenizer
            ...
            # For ref model, use actor's model_config (already loaded)
            # But we still need model_config for padding_free check
            if not hasattr(self, "model_config"):
                # Fallback: load model config if not available
                self.model_config = AutoConfig.from_pretrained(
                    model_config.model_path,
                    trust_remote_code=model_config.trust_remote_code,
                    **model_config.override_config,
        ...
        if padding_free:
            # Ensure model_config is available
            if not hasattr(self, "model_config"):
                self.model_config = AutoConfig.from_pretrained(
                    model_config.model_path,
                    trust_remote_code=model_config.trust_remote_code,
        ...
        if fsdp_config.torch_dtype is None:
            torch_dtype = torch.float32 if role not in ["ref", "reward"] else torch.bfloat16
        else:
        ...
        if role == "critic":
            ...
            # Reward model can be any model type, default to CausalLM
            # If your reward model is a different type, you may need to adjust this
        ...
        if role in ["actor", "critic"]:
            ...
            if optim_config is None:
                raise ValueError(f"Optimizer config is required for {role}.")
            ...
            self.fsdp_module = fsdp_module
            # Reward model doesn't need optimizer
            model.eval()  # Set to evaluation mode
            if self._use_reward_model_param_offload:
                offload_fsdp_model(self.fsdp_module)
                print_gpu_memory_usage(f"After offload {role} model during init")
    ...
    def compute_reward(self, data: DataProto) -> Tuple[torch.Tensor, Dict[str, list]]:
        """Compute reward scores using reward model.
        
        This method is only available when role is "reward".
        ...
        """
        assert self._has_reward_model, "compute_reward is only available for reward model role"
        ...
        with self.ulysses_sharding_manager:
            ...
            with torch.no_grad():
                ...
                if hasattr(outputs, "reward_scores"):
                    ...
                    if logits.dim() == 3:
                        if logits.shape[-1] == 1:
                            reward_scores = logits.squeeze(-1)
                        else:
                            reward_scores = logits.mean(dim=-1)
                    elif logits.dim() == 2:
        ...
        for i in range(batch_size):
            if response_mask is not None:
                sample_rewards = reward_tensor[i]
