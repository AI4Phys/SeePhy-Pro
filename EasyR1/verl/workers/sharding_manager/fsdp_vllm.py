...
def _get_vllm_tp_process_group():
    if hasattr(vllm_ps, "get_tensor_model_parallel_group"):
        ...
        raise AttributeError(
            "Current vLLM version does not expose tensor-parallel group accessors "
            "(expected `get_tensor_model_parallel_group` or `get_tp_group`)."
        )

    return getattr(tp_group, "device_group", tp_group)
...
class FSDPVLLMShardingManager(BaseShardingManager):
    def __init__(
        ...
        self.tp_group = _get_vllm_tp_process_group()
